...
export class OpenAIProvider implements LLMProvider {
    ...
    constructor(apiKey: string, model: string = 'gpt-4o') {
        ...
        this.client = new OpenAI({
            apiKey,
            baseURL: useProxy ? `${proxyUrl}/v1` : undefined,
    ...
    async createMessage(messages: MessageParam[], systemPrompt: string, tools: any[]): Promise<LLMResponse> {
        // Convert Anthropic format messages to OpenAI format if necessary
        // Anthropic: { role: 'user' | 'assistant', content: string | any[] }
        // OpenAI: { role: 'user' | 'assistant' | 'system' | 'tool', content: string | any[], tool_calls?: any, tool_call_id?: any }
